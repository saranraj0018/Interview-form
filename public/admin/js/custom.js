...
async function sendRequest(url, data, method = "POST", onSuccess = null, onError = null) {
    ...
    if (res.status === 422 && responseData.errors) {
        ...
        let firstError = Object.values(responseData.errors)[0][0];

        showToast(firstError, "error", 3000);

    } else {

        showToast(responseData.message || "Something went wrong!", "error", 3000);
...
function showToast(message, type = "success", duration = 3000) {
    const colors = {
        success: "bg-white text-[#ea2498] border border-[#ea2498]",
        error: "bg-white text-red-500 border border-red-500",
        warning: "bg-white text-blue-500 border border-blue-500"
    };

    const icons = {
        success: `<svg class="w-5 h-5 mr-2 inline-block" fill="none" stroke="#ea2498" stroke-width="2" viewBox="0 0 24 24">
